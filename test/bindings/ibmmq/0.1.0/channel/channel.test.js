...
const title = 'Channel'
const validator = TestHelper.validator(require('@bindings/ibmmq/0.1.0/channel.json'))

describe(`${title}`, () => {
  it('queue destination', () => {
    const model = JSON.parse(fs.readFileSync(`${__dirname}/examples/queue destination.json`, 'utf-8'));
    const validationResult = validator(model);

    assert(validationResult === true, `Example MUST be valid`);
  });

  it('topic destination', () => {
    const model = JSON.parse(fs.readFileSync(`${__dirname}/examples/topic destination.json`, 'utf-8'));
    const validationResult = validator(model);
