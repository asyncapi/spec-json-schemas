...
const title = 'Message'
const validator = TestHelper.validator(require('@bindings/ibmmq/0.1.0/message.json'))
...
describe(`${title}`, () => {
  ...
  it('empty', () => {
    ...
    assert(validator.errors[0].message === 'must match exactly one schema in oneOf');
  ...
  it('only required properties', () => {
    ...
    assert(validationResult === false, 'Is not valid with only required properties.');
    assert(validator.errors[0].message === 'must match exactly one schema in oneOf');
    assert(validator.errors.length === 1);
  });
