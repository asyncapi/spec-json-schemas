...
describe('SCRAM-SHA-256', () => {
  ...
  it('empty', () => {
    ...
    assert(validationResult === false, 'SCRAM-SHA-256 with empty body is not valid');
  ...
  it('without required properties', () => {
    ...
    assert(validationResult === false, 'SCRAM-SHA-256 without required properties is not valid');
    assert(validator.errors[0].message === 'must have required property \'type\'');
    assert(validator.errors.length === 1);
  });
  ...
  it('only required properties', () => {
    ...
    assert(validationResult === true, 'SCRAM-SHA-256 is valid with only required properties');
  });
  ...
  it('extended', () => {
    ...
    assert(validationResult === true, 'SCRAM-SHA-256 can be extended');
  });
  ...
  it('wrongly extended', () => {
    ...
    assert(validationResult === false, 'SCRAM-SHA-256 is not valid when was wrongly extended');
