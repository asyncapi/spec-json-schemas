...
describe('X509', () => {
  ...
  it('empty', () => {
    ...
    assert(validationResult === false, 'X509 with empty body is not valid');
  ...
  it('without required properties', () => {
    ...
    assert(validationResult === false, 'X509 without required properties is not valid');
    assert(validator.errors[0].message === 'must have required property \'type\'');
    assert(validator.errors.length === 1);
  });
  ...
  it('only required properties', () => {
    ...
    assert(validationResult === true, 'X509 is valid with only required properties');
  });
  ...
  it('extended', () => {
    ...
    assert(validationResult === true, 'X509 can be extended');
  });
  ...
  it('wrongly extended', () => {
    ...
    assert(validationResult === false, 'X509 is not valid when was wrongly extended');
