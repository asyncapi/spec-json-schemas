...
describe(`${schemaName}`, () => {
  it('empty', () => {
    ...
    assert(validationResult === false, `${schemaName} is not valid with empty body`);
    assert(validator.errors[0].message === 'must have required property \'type\'');
    assert(validator.errors[1].message === 'must have required property \'flows\'');
  ...
  it('without required properties', () => {
    ...
    assert(validationResult === false, `${schemaName} is not valid without required properties`);
    assert(validator.errors[0].message === 'must have required property \'type\'');
    assert(validator.errors[1].message === 'must have required property \'flows\'');
    assert(validator.errors.length === 2);
  });
  ...
  it.skip('wrongly extended. Reason: schema doesn\'t restrict additional properties', () => {
